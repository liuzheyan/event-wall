// 全局配置文件

/** ===== 基础配置 ===== */
const CONFIG = {
  // 弹幕最大长度
  MAX_LEN: 30,
  // 发送冷却时间（毫秒）
  SEND_INTERVAL: 3000,
  // 弹幕轨道数量
  // 增加轨道数以覆盖全屏 (假设 1080p 屏幕，高度 1080 / 56 ≈ 19)
  // 设置为 12
  TRACK_COUNT: 12,
  // 弹幕显示区域垂直占比 (0.0 - 1.0)
  // 0.75 表示只在屏幕顶部 75% 的区域显示弹幕，留出底部给二维码或输入框
  DANMU_AREA_PERCENT: 0.75,
  // 弹幕飘动时长 (秒)
  // 统一时长以保证各端位置基本同步
  DANMU_DURATION: 12,
  // 背景图
  BG_IMAGE: "img/pc_background.jpg",
  // 手机端专用背景图 (如果不设置，默认使用 BG_IMAGE)
  // 建议使用竖屏图片，比例 9:16
  BG_IMAGE_MOBILE: "img/mobile_background.png",
  
  // === 新增需求配置 ===
  // 循环回放检查间隔（默认3分钟 = 180000ms）
  REPLAY_INTERVAL: 3 * 60 * 1000,
  // 触发循环回放的阈值（用户弹幕少于此数量时触发）
  REPLAY_THRESHOLD: 50,

  /** ===== MQTT 配置 (纯静态部署使用) ===== */
  // 使用公共免费 Broker (注意：数据公开，仅用于演示)
  // 你也可以换成自己的 MQTT 服务
  MQTT_BROKER: "wss://broker.emqx.io:8084/mqtt",
  // 随机生成一个 Topic 后缀，避免与他人冲突
  // 如果需要固定，可以改为 "event-wall/my-company"
  // 注意：为了方便本地调试（Mobile 和 PC 打开各自生成随机数会导致连不上），
  // 这里暂时改为固定值。正式使用建议在 screen.html 中动态生成并展示给用户。
  MQTT_TOPIC: "event-wall/public-demo-fixed-channel"

};

/** ===== 违禁词库（已扩展） ===== */
const FORBIDDEN_WORDS = [
  // === 政治类 ===
  "共产党", "中共", "政府", "独裁", "民主", "人权", "六四", "8964", 
  "法轮功", "台独", "藏独", "疆独", "港独", 
  "习近", "李克", "毛泽", "邓小", "江泽", "胡锦", "习近平", "李克强",
  "反动", "暴乱", "起义", "示威", "游行", "罢工", "信访", "上访", "维权", 
  "洗脑", "邪教", "恐怖分子",

  // === 脏话/辱骂类 ===
  "傻逼", "SB", "sb", "Sb", "2B", "二逼", "脑残", "智障", "白痴", "蠢猪", 
  "狗日", "妈的", "他妈", "你妈", "NMD", "nmd", "TMD", "tmd", "WCNM", "wcnm", "cnmd", "wdnmd",
  "草泥马", "操你", "fuck", "shit", "bitch", "whore", "asshole", 
  "笨蛋", "傻瓜", "坏蛋", "垃圾", "废物", "去死", "滚蛋", "滚", 
  "死全家", "全家死", "尼玛", "傻X", "煞笔", "弱智", "吃屎", "混蛋", "脑瘫", "畜生", "婊子", "贱人",

  // === 公司/职场负面 (原有保留) ===
  "领导傻", "公司垃圾", "裁员", "倒闭", "坑爹", "吸血", "剥削", "黑心",
  "辞退", "降薪", "欠薪", "仲裁", "劳动局", "傻叉领导", "画大饼", "996", "ICU",

  // === 黄赌毒/暴力/其他 ===
  "暴力", "血腥", "色情", "赌博", "诈骗", "AV", "三级片", "强奸", "迷药", 
  "枪支", "毒品", "海洛因", "冰毒", "k粉", "摇头丸", "大麻", "炸弹",
  "黄色", "做爱", "约炮", "裸聊", "兼职", "刷单"
];

/** ===== 默认年会弹幕库 ===== */
const DEFAULT_DANMU = [
  "🎉 年会正式开始啦！",
  "👏 辛苦一年，掌声送给自己",
  "✨ 今晚不加班，只加快乐",
  "🥂 欢迎来到我们的年会现场",
  "🔥 氛围组已就位！",
  "😄 今天的关键词：开心",
  "🚀 新的一年一起冲！",
  "💼 感谢每一位同事的付出",
  "🎊 年会氛围拉满！",
  "🙌 全员集合，准备嗨！",
  "🎁 抽奖马上开始，祝大家好运",
  "🍀 锦鲤就在现场",
  "😎 手气这种事，说来就来",
  "🧧 希望下一位是我",
  "🤞 已经开始默默许愿",
  "🪄 好运正在加载中…",
  "🎯 抽奖就是心跳加速器",
  "🥳 恭喜所有中奖的小伙伴",
  "😭 没中奖也要保持微笑",
  "🎁 重在参与，开心最重要",
  "💪 团队协作，天下无敌",
  "🌟 每一份努力都值得被看见",
  "🏆 为优秀的同事点赞",
  "🤝 感谢并肩作战的伙伴们",
  "🧠 今天不聊 KPI",
  "💡 明天继续发光",
  "🏗️ 一起把事情做好",
  "🌈 未来可期",
  "📈 越来越好，不是口号",
  "🥇 我们是最棒的团队",
  "😄 气氛已经到位",
  "🥂 今晚不谈工作",
  "💤 明天的事明天再说",
  "🎶 音乐一响，快乐登场",
  "😆 弹幕比我反应快",
  "👀 大屏幕好热闹",
  "🎈 氛围正在升温",
  "🫶 快乐是会传染的",
  "😎 这就是年会的感觉",
  "🌙 今晚值得记住",
  "🎆 精彩还在继续",
  "🙏 感谢所有幕后工作人员",
  "🎉 今年也辛苦大家了",
  "💖 把掌声送给彼此",
  "🌟 这一刻很珍贵",
  "🎊 快乐请继续",
  "🥰 今天真的很开心",
  "🚀 新一年，一起更好",
  "🍻 为这一年干杯",
  "🎉 年会圆满进行中！",
  "🎉 2026年会快乐！", 
  "业绩长虹，再创辉煌！", 
  "奖金拿到手软！", 
  "大家吃好喝好！", 
  "祝大家马年快乐、马到成功！", 
  "现场气氛太棒了！",
  "马年，马上有钱！",
  "一马当先，赢全年！",
  "跟着老板，策马奔腾！",
  "年会 YYDS！",
  "辞旧迎新，马年好运！",
  "马年行大运，事事如意！",
  "搞钱要紧，马年大吉！",
  " 蒸蒸日上，马年更火！",
  "龙马精神，万事胜意！",
  "新年快乐，红包拿来！",
  "团队一心，其利断金！",
  "2026，我们顶峰相见！",
  "嗨翻全场，今晚不醉不归！",
  "星光璀璨，未来可期！",
  "平平安安，顺顺利利！",
  "工作顺利，升职加薪！",
  "假期多多，快乐加倍！",
  "感谢公司，感恩有你！",
  "效率翻倍，早点下班！",
  "每天都有好心情！",
  "欧气满满，中大奖！",
  "策马扬鞭，再创佳绩！",
  "龙马精神，光耀全场！",
  "暴富暴瘦，啥都有！",
  "薪水翻倍，快乐加倍！",
  "存款后面加个零！",
  "最佳拍档，就是我们！",
  "公司是我家，发财靠大家！",
  " 燃烧卡路里，冲刺KPI！",
  "销冠预定，舍我其谁！",
  "今朝有酒今朝醉！",
  "吃好喝好，快乐没烦恼！",
  "拍照记得开美颜！",
  "保持热爱，奔赴山海！",
  "🙏 求大奖！求大奖！",
  " 马年要暴富！",
  " 老板发红包！",
  " 听懂掌声！",
  " 为梦想窒息！",
  " 这也太酷了吧！",
  " 疯狂打 call！",
  " 爱了爱了！",
  " 全场最靓的仔！",
  " C 位出道！",
  "颜值爆表！",
  " 666666",
  " 必须给力！",
  "奥力给！",
  " 冲鸭！！！",
  " 未来可期，不负韶华！",
  " 以梦为马，不负韶华！",
  " 马不停蹄，再接再厉！",
  " 万马奔腾，气势如虹！",
  " 马上有对象！",
  " 马上有房！",
  " 马上有车！",
  " 马上有喜！",
  "马上成功！",
  " 马上加薪！",
  " 马上瘦身成功！",
  " 马上脱单！",
  " 马上暴富！",
  " 马上开心！",
  " 咱们公司藏龙卧虎！",
  "还有谁？！",
  "👯 气氛组已就位！",
  " 妈妈我上电视了！",
  " 一位靓仔路过~",
  " 弹幕护体！",
  "祝大家发财！",
  "明年再创辉煌！",
  "让我们红尘作伴，活得潇潇洒洒！",
  "对酒当歌，人生几何！",
  
];

/** ===== 工具函数：文本过滤与检查 ===== */
function filterText(text) {
  let result = text;
  FORBIDDEN_WORDS.forEach(word => {
    try {
      const reg = new RegExp(word, "ig");
      result = result.replace(reg, "***");
    } catch (e) {
      // 忽略正则错误
    }
  });
  return result;
}

/**
 * 检查是否包含敏感词
 * @param {string} text 
 * @returns {boolean}
 */
function hasSensitive(text) {
  return FORBIDDEN_WORDS.some(word => {
    try {
      // 简单的包含判断，忽略大小写
      const reg = new RegExp(word, "i");
      return reg.test(text);
    } catch (e) {
      return false;
    }
  });
}
